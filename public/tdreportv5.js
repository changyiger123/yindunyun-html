 ...
 */
$.extend({
  showReport1: function (data, name1) {
    var displayName = {
      "ANTIFRAUD": "银联信用",
      "ANTIFRAUD_INFOQUERY": "基础数据",
      "AUTHENTICATION": "信息核验",
      "CREDIT": "授信定价",
      "WITHDRAWALS": "提现",
      "PREFILTER": "预筛",
    ...
    var templateData1 = dealData(data, name1);
          ...
          for (var result in item.result_desc) {
            if (result == "INFOANALYSIS") {
              $.extend(resData, {"INFOANALYSIS": item.result_desc.INFOANALYSIS});
                ...
                if(tmp.final_score || tmp.final_score == 0) {
                  if (0 <= tmp.final_score && tmp.final_score < 40){
                    tmp.final_decision = "PASS"
                  } else if (40 <= tmp.final_score && tmp.final_score < 80) {
                    tmp.final_decision = "REVIEW"
                  } else if (80 <= tmp.final_score) {
                    tmp.final_decision = "REJECT"
                  }
                }
    ...
    function addCSSFile() {
      ...
      if (!isAdd) {
        ...
        $.each($('script'), function (index, item) {
          var js_index = $(item).prop('src').indexOf('tdreportv5');
          if (js_index > -1) {
    ...
    var render = template.compile(sourceReportTemplate1);
    var html = render(templateData1);
