...
class applicationForm extends React.Component {
    ...
    componentWillMount= (e) =>{
        ...
        ajax.post("/admin/autoloanApply/last/audit/list",redata)
            .then(response =>{
                if(response.code=="0"){
    ...
    handleSubmit1 = (e) => {
        ...
        this.props.form.validateFields((err, values) => {
            if (!err) {
                ...
                ajax.post("/admin/autoloanApply/last/audit/list",data1)
                    .then(response =>{
                        if(response.code=="0"){
    ...
    render() {
        ...
        const columns1 = [{
            ...
            dataIndex: 'operation',
            key: 'operation',
            render: (text, record) =><Link to={'/auditList/detailOpinion4/' + record.applyNo}>审核</Link>
        }];
        const columns2 = [{
            title: '申请单号',
            dataIndex: 'applyNo',
        ...
        return(
            <div className="distributionList staffManage">
                <Breadcrumb style={{ padding: '16px 28px',background:"#fff"}}>
                    <Breadcrumb.Item>终审列表</Breadcrumb.Item>
                </Breadcrumb>
                {/*查询选项*/}
