...
class applicationForm extends React.Component {
        ...
        ajax.post("/admin/autoloanApply/info",{applyNo:this.state.applyNo})
            .then(response =>{
                if(response.code=="0") {
                    ...
                    var str =data.riskControlReport;
                    console.log(str);
                    var t = 248,
                        _lenght = str.length;
                    if( _lenght > t ){
    ...
    handleSubmit = (e) => {
        ...
        this.props.form.validateFields((err, values) => {
            if (!err) {
                ...
                ajax.post("/admin/autoloanApply/second/verify/passOrNo/update",data)
                    .then(response =>{
                        console.log(response);
                        if(response.code=="0"){
                            message.success("提交成功！");
                            this.props.router.push('/auditList/list3');
                        }else{
                            console.log("list"+response.msg);
                            message.error(response.msg);
                        }
    ...
    handleOk = () => {
        ...
        if(supplement){
            ajax.post("/admin/autoloanApply/second/verify/suplement/update",{verifySupplementContent:supplement,applyNo:applyNo})
                .then(response =>{
                    if(response.code=="0") {
                        this.props.router.push('/auditList/list3');
                    }else{
        ...
        return(
            <div className="auditList">
                ...
                <div id="mainCont" style={{marginTop:"24px"}}>
                    ...
                    {/*复审意见*/}
                    <div className="infoBox opration" style={{margin:"0 24px 80px"}}>
                        <Form onSubmit={this.handleSubmit}>
                            ...
                            <FormItem label="备注/不通过理由：">
                              {getFieldDecorator("verifyReport", {
                                  ...
                                  rules: [
                                      {required: true, message: "请输入备注/不通过理由！"}
                                  ]
                              })(
                            ...
                            <Row style={{backgroundColor: '#fff',width:"calc(100% - 200px)",padding:"13px 32px",position:"fixed",bottom:"0",left:"200px",borderTop:"1px solid rgb(232, 232, 232)"}}>
                                ...
                                <Col className="rightBtn" style={{float: "right"}}>
                                    <Button><Link to={'/auditList/list3'}>取消</Link></Button>
