...
class applicationForm extends React.Component {
    ...
    handleSubmit = (e) => {
        ...
        this.props.form.validateFields((err, values) => {
            if (!err) {
                ...
                ajax.post("/admin/autoloanApply/all/list",data)
                    .then(response =>{
                        console.log(response);
                        if(response.code=="0"){
                            var datalist=response.result,
    ...
    render() {
        ...
        const columns = [{
            ...
            dataIndex: 'operation',
            key: 'operation',
            render: (text, record) => <Link to={record.operation == "0" ? '/auditList/detail/' + record.applyNo  : '/auditList/supplemen/' + record.applyNo} name="operate">{record.operation == "0" ? "详情" : "编辑"}</Link>
        }];
                ...
                <div id="mainCont" style={{ margin: 24,padding:'0 32px', background: '#fff', minHeight: 780 }}>
                   ...
                   {/* <Button type="primary"><Icon type="plus" /><Link to="/newApplication/newEdit">新建</Link></Button>*/}
                    <div className="ant-form-split">
                        <Table className="listTable" rowKey={(record) => record.id} columns={columns} dataSource={dataCont} pagination={{
                            simple:false,
