...
import "../newApplication/newApplication.less";
...
class applicationEditForm extends React.Component {
    state = {
        ...
        cardDate:["2018-08-08","2023-08-07"],//身份证有效期
    ...
    toCaculator(obj){
        if(obj == "autoPrice" || obj == "assessPrice" || obj == "loadMoney" ||obj == "signedApr" ||obj == "bankApr" ||obj == "periodsId"||obj == "loadBankCode" ){
            ...
            if(autoPrice != undefined && loadMoney != undefined && signedApr != undefined && bankApr!= undefined && periodsId!= undefined && loadBankCode!= undefined ){
                ajax.post("/admin/loanInfo/caculator",{autoPrice:autoPrice,loadMoney:loadMoney,signedApr:signedApr,bankApr:bankApr,periodsId:periodsId,loadBankCode:loadBankCode})
                    .then(response =>{
    ...
    componentDidMount = () => {
        ...
        if (applyNo) {
            ajax.post("/admin/autoloanApply/info", {applyNo: this.props.params.id})
                .then(response => {
                    if (response.code == "0") {
                        ...
                        if(cardFrontPic){
                            let {uid,status,name,url,typeId}={uid:0,status:'done',name:'其他',url:cardFrontPic,typeId:-1};
                        ...
                        if(cardBackPic){
                            let {uid,status,name,url,typeId}={uid:0,status:'done',name:'其他',url:cardBackPic,typeId:-1};
                        ...
                        if(authorizedPic){
                            let {uid,status,name,url,typeId}={uid:0,status:'done',name:'其他',url:authorizedPic,typeId:-1};
                        ...
                        if(signAuthorizedPic){
                            let {uid,status,name,url,typeId}={uid:0,status:'done',name:'其他',url:signAuthorizedPic,typeId:-1};
                        ...
                        if(creditUrl){
                            for(var i=0;i<creditUrl.length;i++){
                                let {uid,status,name,url,typeId}={uid:i,status:'done',name:'其他',url:creditUrl[i],typeId:-1};
                        ...
                        if(spousePic){
                            for(var i=0;i<spousePic.length;i++){
                                let {uid,status,name,url,typeId}={uid:i,status:'done',name:'其他',url:spousePic[i],typeId:-1};
                        ...
                        if(guaranteePic){
                            for(var i=0;i<guaranteePic.length;i++){
                                let {uid,status,name,url,typeId}={uid:i,status:'done',name:'其他',url:guaranteePic[i],typeId:-1};
                        ...
                        if(guaranteePic2){
                            for(var i=0;i<guaranteePic2.length;i++){
                                let {uid,status,name,url,typeId}={uid:i,status:'done',name:'其他',url:guaranteePic2[i],typeId:-1};
                        ...
                        this.setState({
                            ...
                            sex: data.sex==undefined?'':data.sex.toString(),
                            ...
                            applyEmployeeName: data.applyEmployeeName,
            ...
            this.props.router.push('/auditList/list1');
        }
    ...
    handleSubmit = (e) => {
        ...
        this.props.form.validateFields((err, values) => {
            if (!err) {
                if (this.state.loadMoney && this.state.autoType == "0" && ((this.state.autoPrice - 0) < (this.state.loadMoney - 0))) {
                    ...
                    ajax.post("/admin/autoloanApply/supplement/update", data)
                        .then(response => {
                            console.log(response);
                            if (response.code == "0") {
                                // var data=response.data;
    ...
    formSave = (e) => {
        ...
        this.props.form.validateFields((err, values) => {
            if (!err) {
                if (this.state.creditContent == undefined && this.state.creditUrlList.length<1) {
                    ...
                    ajax.post("/admin/autoloanApply/supplement/save", data)
                        .then(response => {
                            console.log(response);
    ...
    moreMsg = () => {
        if($('.moremsg').text() == '显示全部') {
            this.setState({
                info_count: this.state.auditInfo.length
            })
            $('.moremsg').text('收起');
        } else {
            this.setState({
    ...
    render() {
        ...
        const  auditInfo  = this.state.auditInfo || {};
        ...
        for (let i = 0; i < auditInfo_length; i++) {
            auditInfoList.push(
                <div className="cont" id={"cont"+i} key={i} style={{marginTop:"10px"}}>
                    <h3 style={{width:'49%',display:'inline-block'}}>{auditInfo[i].title==null?"" :auditInfo[i].title}</h3>
                    <p className="salesMan" style={{width:'49%',display:'inline-block',textAlign:'right'}}>
                        <span style={{marginTop:"0"}}><i>{auditInfo[i].roleName}:</i>{auditInfo[i].auditor==undefined?"" :auditInfo[i].auditor}</span>
                        <span style={{marginRight:'0',marginTop:"0"}}><i>审核时间：</i>{auditInfo[i].auditTime==undefined?"" :auditInfo[i].auditTime}</span>
                    </p>
                    <span className="textCont packup" style={{display:"block",wordWrap:'break-word',lineHeight:'22px',marginTop:"0"}}>
                        {auditInfo[i].content==undefined?"" :auditInfo[i].content}
                    </span>
        ...
        return (
            <div className="newApplication auditList" style={{position: "relative"}}>
                ...
                <div className="mainInfo clearfix">
                    ...
                    <div id="audit" className="contLeft">
                        <p className="salesMan" style={{marginTop:"0"}}>
                            <span style={{marginTop:"0"}}><i>业务员：</i>{this.state.applyEmployeeName}</span>
                            <span style={{marginTop:"0"}}><i>申请时间：</i>{this.state.applyDate}</span>
                        </p>
                        {auditInfoList}
                    </div>
                    <div className="contRight clearfix">
                        <i>状态</i>
                        <h2>{this.state.statusStr}</h2>
                    </div>
                    {auditInfo.length > 3?
                ...
                <div id="mainCont" style={{margin: '24px', padding: '0 20px 20px', background: '#fff', minHeight: 780}}>
                    ...
                    <Form className="editForm" horizontal="true" onSubmit={this.handleSubmit}>
                        ...
                        <div id="info">
                            ...
                            <FormItem
                                ...
                                {getFieldDecorator("realName", {
                                    ...
                                    <Input type="text" className="ant-form-text" id="realName" name="realName" placeholder="请输入用户名/手机号"
                            ...
                            <FormItem
                                ...
                                {getFieldDecorator("birthday", {
                                    ...
                                    rules: [
                                        {required: false, message: "请选择出生年月！"}
                                    ]
                                })(
                            ...
                            <FormItem
                                ...
                                {getFieldDecorator("cardId", {
                                    ...
                                    rules: [
                                        {required: true,
                                            pattern:"(^\\d{6}(18|19|20)\\d{2}(0[1-9]|1[012])(0[1-9]|[12]\\d|3[01])\\d{3}(\\d|[xX])$|^\\d{6}\\d{2}(0[1-9]|1[012])(0[1-9]|[12]\\d|3[01])\\d{3}$)",message: "请输入身份证号码！"}
                                    ]
                                })(
                            ...
                            <FormItem
                                ...
                                {getFieldDecorator("cardStartDate", {
                                    ...
                                    rules: [
                                        {required: false, message: '请选择身份证开始日期'}
                                    ]
                                })(
                            ...
                            <FormItem
                                ...
                                {getFieldDecorator("permanentAddress", {
                                    ...
                                    rules: [
                                        {required: false, message: "请输入户籍地址！"}
                                    ]
                                })(
                            ...
                            <FormItem
                                ...
                                <div className="picContent clearfix">
                                    ...
                                    <Modal className="supplemenModal" visible={previewVisible} footer={null} onCancel={this.handleCancel}>
                            ...
                            >
                                {getFieldDecorator("creditContent", {
                                    ...
                                    rules: [
                                        {required: false, message: "请输入征信情况！"}
                                    ]
                                })(
                                    <textarea placeholder="征信情况" className="ant-input" name="creditContent"
                            ...
                            <FormItem
                                ...
                                {getFieldDecorator("aboutCourt", {
                                    ...
                                    rules: [
                                        {required: false, message: "请输入法院情况！"}
                                    ]
                                })(
                                    <textarea placeholder="默认显示征信人提供的资料，内勤可编辑" className="ant-input" name="aboutCourt"
                            ...
                            <div style={{display: formData.isMarried == 1 ? 'block' : 'none'}}>
                                <FormItem
                                    ...
                                    {getFieldDecorator("spouseRealName", {
                                        ...
                                        rules: [
                                            {required: false, message: "请输入配偶姓名！"}
                                        ]
                                    })(
                                ...
                                <FormItem
                                    ...
                                    {getFieldDecorator("spouseMobilePhone", {
                                        ...
                                        rules: [
                                            {required: false, pattern:"^1[3|4|5|7|8|9]\\d{9}$", message: "请输入正确的配偶手机号！"}
                                        ]
                                    })(
                                ...
                                <FormItem
                                    ...
                                    {getFieldDecorator("spouseCardId", {
                                        ...
                                        rules: [
                                            {required: false,
                                                pattern:"(^\\d{6}(18|19|20)\\d{2}(0[1-9]|1[012])(0[1-9]|[12]\\d|3[01])\\d{3}(\\d|[xX])$|^\\d{6}\\d{2}(0[1-9]|1[012])(0[1-9]|[12]\\d|3[01])\\d{3}$)",message: "请输入正确的配偶身份证号码！"}
                                        ]
                                    })(
                                ...
                                <FormItem
                                    ...
                                    {getFieldDecorator("spousePicList", {
                                        ...
                                        rules: [
                                            {required: false, message: "请上传配偶照片！"}
                                        ]
                                    })(
                                        <div className="picContent clearfix cridtof">
                                            <Upload
                            ...
                            <FormItem
                                ...
                                {getFieldDecorator("haveCommonBorrower", {
                                    ...
                                    rules: [
                                        {required: false, message: "请选择是否有共同借款人！"}
                                    ]
                                })(
                            ...
                            <FormItem
                                ...
                                {getFieldDecorator("haveGuarantee", {
                                    ...
                                    rules: [
                                        {required: false, message: "请选择是否有担保人！"}
                                    ]
                                })(
                            ...
                            <div style={{display: formData.haveGuarantee == 1 ? 'block' : 'none'}}>
                                <FormItem
                                    ...
                                    {getFieldDecorator("guaranteeRealName", {
                                        ...
                                        rules: [
                                            {required: false, message: "请输入担保人姓名！"}
                                        ]
                                    })(
                                ...
                                <FormItem
                                    ...
                                    {getFieldDecorator("guaranteeMobilePhone", {
                                        ...
                                        rules: [
                                            {
                                                required: false,
                                                pattern:"^1[3|4|5|7|8|9]\\d{9}$", message: "请输入正确的担保人手机号！"
                                            }
                                        ]
                                    })(
                                ...
                                <FormItem
                                    ...
                                    {getFieldDecorator("guaranteeMobilePhone2", {
                                        ...
                                        rules: [
                                            {
                                                required: false,
                                                pattern:"^1[3|4|5|7|8|9]\\d{9}$", message: "请输入正确的第二担保人手机号！"
                                            }
                                        ]
                                    })(
                            ...
                            <FormItem
                                ...
                                {getFieldDecorator("haveFirstContact", {
                                    ...
                                    rules: [
                                        {required: false, message: "请选择是否有其他联系人一！"}
                                    ]
                                })(
                            ...
                            <FormItem
                                ...
                                {getFieldDecorator("haveSecondContact", {
                                    ...
                                    rules: [
                                        {required: false, message: "请选择是否有其他联系人二！"}
                                    ]
                                })(
                        ...
                        <div id="borrowInfo">
                            ...
                            <FormItem
                                ...
                                {getFieldDecorator("autoType", {
                                    ...
                                    rules: [
                                        {required: false, message: "请选择车辆类型！"}
                                    ]
                                })(
                            ...
                            <FormItem
                                ...
                                {getFieldDecorator("autoInsurance", {
                                    initialValue: formData.autoInsurance,
                                    rules: [
                                        {required: false, message: "请选择车辆保险！"}
                                    ]
                                })(
                            ...
                            {this.state.autoType == '1' ? 
                                <FormItem
                                    ...
                                    {getFieldDecorator("assessPrice", {
                                        ...
                                        rules: [
                                            {required: false, message: "请输入二手车评估价"}
                                        ]
                                    })(
                                ...
                                <FormItem
                                    ...
                                    {getFieldDecorator("autoPrice", {
                                        ...
                                        rules: [
                                            {required: false, message: "请输入车辆价格！"}
                                        ]
                                    })(
                            ...
                            <FormItem
                                ...
                                {getFieldDecorator("loadMoney", {
                                    ...
                                    rules: [
                                        {required: false, message: "请输入贷款金额！"}
                                    ]
                                })(
                            ...
                            <FormItem
                                ...
                                {getFieldDecorator("firstMoney", {
                                    ...
                                    rules: [
                                        {required: false, message: "请输入首付款金额！"}
                                    ]
                                })(
                            ...
                            <FormItem
                                ...
                                {getFieldDecorator("firstScale", {
                                    ...
                                    rules: [
                                        {required: false, message: "请输入首付比例！"}
                                    ]
                                })(
                            ...
                            <FormItem
                                ...
                                {getFieldDecorator("periodsId", {
                                    ...
                                    rules: [
                                        {required: false, message: "请输入贷款期数！"}
                                    ]
                                })(
                            ...
                            <FormItem
                                ...
                                {getFieldDecorator("loadBankCode", {
                                    ...
                                    rules: [
                                        {required: false, message: "请输入贷款银行！"}
                                    ]
                                })(
                            ...
                            <FormItem
                                ...
                                {getFieldDecorator("bankApr", {
                                    ...
                                    rules: [
                                        {required: false, message: "请输入银行利率！"}
                                    ]
                                })(
                            ...
                            <FormItem
                                ...
                                {getFieldDecorator("signedLoadMoney", {
                                    ...
                                    rules: [
                                        {required: false, message: "请输入签约贷款额！"}
                                    ]
                                })(
                            ...
                            <FormItem
                                ...
                                {getFieldDecorator("signedFirstMoney", {
                                    ...
                                    rules: [
                                        {required: false, message: "请输入签约首付！"}
                                    ]
                                })(
                            ...
                            <FormItem
                                ...
                                {getFieldDecorator("signedFirstScale", {
                                    ...
                                    rules: [
                                        {required: false, message: "请输入签约首付比例！"}
                                    ]
                                })(
                            ...
                            <FormItem
                                ...
                                {getFieldDecorator("moneyPerMonth", {
                                    ...
                                    rules: [
                                        {required: false, message: "请输入月还款金额！"}
                                    ]
                                })(
                            ...
                            <FormItem
                                ...
                                {getFieldDecorator("loadScale", {
                                    ...
                                    rules: [
                                        {required: false, message: "请输入贷款比例！"}
                                    ]
                                })(
                            ...
                            <FormItem
                                ...
                                {getFieldDecorator("applyLoadMoney", {
                                    ...
                                    rules: [
                                        {required: false, message: "请输入申请调整账户额度！"}
                                    ]
                                })(
                        ...
                        <div id="orderInfo">
                            ...
                            <FormItem
                                ...
                                {getFieldDecorator("applyEmployeeCode", {
                                    initialValue: formData.applyEmployeeCode,
                                    rules: [
                                        {required: false, message: "请选择业务员"}
                                    ]
                                })(
                        ...
                        {/*按钮 撤销 提交 保存 取消*/}
                        <Row className="bottom_btn" style={{width: 'calc(100% - 200px)',position:"fixed",bottom:"0", borderTop:"1px solid rgb(232, 232, 232)"}}>
                            <Col style={{float: "left"}}>
                                <Button type="ghost" onClick={this.formRevoke} >撤销</Button>
                            </Col>
                            <Col className="rightBtn" style={{float: "right"}}>
                                <Button><Link to={'/newApplication/list'}>取消</Link></Button>
