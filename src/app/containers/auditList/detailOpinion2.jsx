...
class applicationForm extends React.Component {
    ...
    handleSubmit = (e) => {
        ...
        this.props.form.validateFields((err, values) => {
            if (!err) {
                ...
                ajax.post("/admin/autoloanApply/first/verify/passOrNo/update",data)
                    .then(response =>{
                        console.log(response);
                        if(response.code=="0"){
                            message.success("提交成功！");
                            this.props.router.push('/auditList/list2');
                        }else{
                            console.log("list"+response.msg);
                            message.error(response.msg);
                        }
    ...
    handleOk = () => {
        ...
        if(supplement){
            ajax.post("/admin/autoloanApply/first/verify/suplement/update",{verifySupplementContent:supplement,applyNo:applyNo})
                .then(response =>{
                    if(response.code=="0") {
                        this.props.router.push('/auditList/list2');
                    }else{
                        ...
                        <Form onSubmit={this.handleSubmit}>
                            ...
                            <FormItem label="风控报告：">
                              {getFieldDecorator("verifyReport", {
                                  ...
                                  rules: [
                                      {required: true, message: "请输入风控报告！"}
                                  ]
                              })(
                                <textarea style={{height:"200px",width:"100%"}} placeholder="请输入风控报告" className="ant-input" name="verifyReport" rows="3"  onChange={this.onChange.bind(this, 'verifyReport')}></textarea>
                              )}
                            </FormItem>
                            <Row style={{backgroundColor: '#fff',width:"calc(100% - 200px)",padding:"13px 32px",position:"fixed",bottom:"0",left:"200px",borderTop:"1px solid rgb(232, 232, 232)"}}>
                                ...
                                <Col className="rightBtn" style={{float: "right"}}>
                                    <Button><Link to={'/auditList/list2'}>取消</Link></Button>
