...
class EquipmentList extends React.Component {
    ...
    myshowReport (i,e) {
        ajax.post('/admin/deviceUseInfo/info', {code:i})
        .then(response => {
            if(response.code == "0") {
                this.setState({
                    previewVisible: true,
    ...
    getList () {
        ...
        ajax.post("/admin/deviceUseInfo/list",{mobile: this.state.mobilePhone, addTimeBegin: this.state.startApplyDate1up, addTimeEnd: this.state.endApplyDate1up, page: this.state.page1})
        .then(response => {
            if(response.code == "0") {
        ...
        return(
            <div className="distributionList staffManage">
                <Breadcrumb style={{ padding: '16px 28px',background:"#fff"}}>
                    <Breadcrumb.Item>手机行为画像查询记录</Breadcrumb.Item>
                </Breadcrumb>
                {/*查询选项*/}
...
const equipmentList = Form.create()(EquipmentList);
