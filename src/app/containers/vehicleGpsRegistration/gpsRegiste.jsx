...
class applicationForm extends React.Component {
    state = {
        ...
        gpsCode:"",
        dataTemp:{realName:'',mobilePhone:'', applyNo:'',
            applyEmployeeName:'', autoType:'', loadMoney:'', loanBankName:'',
            storeName:'', applyDate:''},
    ...
    componentWillMount= (e) =>{
        ...
        ajax.post("/admin/autoloanApply/notGps/list",redata)
            .then(response =>{
                if(response.code=="0"){
        ...
        ajax.post("/admin/autoloanApply/hasGps/list",data2)
            .then(response =>{
                if(response.code=="0"){
                    var datalist2=response.result;
                    var totalCount=response.totalCount;
                    console.log("总数为2：",totalCount);
                    this.setState({
                        data2:datalist2,
                        total2:totalCount,
                    });
    ...
    handleSubmit1 = (e) => {
        ...
        this.props.form.validateFields((err, values) => {
            if (!err) {
                ...
                ajax.post("/admin/autoloanApply/notGps/list",data1)
                    .then(response =>{
                        if(response.code=="0"){
    ...
    handleSubmit2 = (e) => {
        ...
        this.props.form.validateFields((err, values) => {
            if (!err) {
                ...
                ajax.post("/admin/autoloanApply/hasGps/list",data)
                    .then(response =>{
        ...
        if(gpsCode){
            ajax.post("/admin/autoloanApply/gps/register",{gpsCode:gpsCode,applyNo:applyNoGps})
                .then(response =>{
                    console.log(response);
                    if(response.code=="0") {
                        var srcData=this.state.data1;
                        var resultData=[];
                        for(var i=0;i<srcData.length;i++){
                            if(srcData[i].applyNo!=this.state.dataTemp.applyNo){
                                resultData.push(srcData[i]);
                            }
                        }
                        this.setState({
                            data1:resultData,
                            total1:resultData.length,
                            gpsCode:'',
                        })
                        console.log("currentPage:",resultData.length%10);
                        if(resultData.length%10==0&&this.state.page1!=1){
                            /* this.setState({
                                page1:this.state.page1-1,
                             });*/
                            this.changPage1(this.state.page1-1);
                        }
    ...
    formGpsCode = (e,record) =>{
        console.log("record is haha:",record);
        console.log("record is isisis",e);
        var {realName,mobilePhone, applyNo, applyEmployeeName, autoType, loadMoney, loanBankName, storeName, applyDate}=e;
        console.log("first:",realName,mobilePhone);
        this.setState({
            visible: true,
            applyNoGps:e.applyNo,
            dataTemp:{realName:realName,mobilePhone:mobilePhone, applyNo:applyNo,
                applyEmployeeName:applyEmployeeName, autoType:autoType, loadMoney:loadMoney, loanBankName:loanBankName,
                storeName:storeName, applyDate:applyDate},
        });
        //dataTemp:{realName,mobilePhone, applyNo, applyEmployeeName, autoType, loadMoney, loanBankName, storeName, applyDate};
        console.log("tempdata:",this.state.dataTemp);
    };
    changPage1=(page)=>{
    ...
    render() {
        ...
        const columns1 = [{
            ...
            dataIndex: 'operation',
            key: 'operation',
            render:(text,record)=><div style={{color:'#1890ff', cursor:"pointer"}} onClick={this.formGpsCode.bind(this,record)}>登记</div>
        }];
        const columns2 = [{
            title: '申请单号',
            dataIndex: 'applyNo',
        ...
        return(
            <div className="distributionList staffManage">
                <Breadcrumb style={{ padding: '16px 28px',background:"#fff"}}>
                    <Breadcrumb.Item>车辆GPS登记</Breadcrumb.Item>
                </Breadcrumb>
                {/*查询选项*/}
                <Tabs defaultActiveKey="1" onChange={this.callback.bind(this)}>
                    <TabPane tab="未登记" key="1">
