...
class SocialSecurityList extends React.Component {
    state = {
        startValue1: null,
        endValue1: null,
        data1: [],
    ...
    myshowReport (i,e) {
        ajax.post('/admin/rcQueryLog/socialSecurity/info', {wfid:i})
        .then(response => {
            if(response.code == "0") {
                this.setState({
                    previewVisible: true,
    ...
    getList () {
        ...
        ajax.post("/admin/rcQueryLog/list",{realName: this.state.realName, addTimeBegin: this.state.startApplyDate1up, addTimeEnd: this.state.endApplyDate1up, page: this.state.page1,type:4})
        .then(response => {
            if(response.code == "0") {
        ...
        return(
            <div className="distributionList staffManage">
                <Breadcrumb style={{ padding: '16px 28px',background:"#fff"}}>
                    <Breadcrumb.Item>社保查询记录</Breadcrumb.Item>
                </Breadcrumb>
                {/*查询选项*/}
...
const socialSecurityList = Form.create()(SocialSecurityList);
