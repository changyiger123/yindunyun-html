...
class AccumulationFundList extends React.Component {
    state = {
        startValue1: null,
        endValue1: null,
        data1: [],
    ...
    myshowReport (i,e) {
        ajax.post('/admin/rcQueryLog/providentFund/info', {wfid:i})
        .then(response => {
            if(response.code == "0") {
                this.setState({
                    previewVisible: true,
    ...
    getList () {
        ...
        ajax.post("/admin/rcQueryLog/list",{realName: this.state.realName, addTimeBegin: this.state.startApplyDate1up, addTimeEnd: this.state.endApplyDate1up, page: this.state.page1,type:3})
        .then(response => {
            if(response.code == "0") {
    ...
    render() {
        const columns1 = [{
            ...
            dataIndex: 'operation',
            key: 'operation',
            render: (text, record) =><a onClick={this.myshowReport.bind(this,record.rcNo)} disabled={record.isValid}>{record.isValid? '-' : '查看'}</a>
        }];
        ...
        return(
            <div className="distributionList staffManage">
                <Breadcrumb style={{ padding: '16px 28px',background:"#fff"}}>
                    <Breadcrumb.Item>公积金查询记录</Breadcrumb.Item>
                </Breadcrumb>
                {/*查询选项*/}
...
const accumulationFundList = Form.create()(AccumulationFundList);
export default accumulationFundList;