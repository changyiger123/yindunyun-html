...
class applicationForm extends React.Component {
    ...
    getData(){
        ...
        ajax.post("/admin/autoloanApply/notSend/list",redata1)
            .then(response =>{
                if(response.code=="0"){
        ...
        ajax.post("/admin/autoloanApply/hasSend/list",redata2)
            .then(response =>{
                if(response.code=="0"){
    ...
    handleSubmit1 = (e) => {
        ...
        this.props.form.validateFields((err, values) => {
            if (!err) {
                ...
                ajax.post("/admin/autoloanApply/notSend/list",data1)
                    .then(response =>{
    ...
    handleSubmit2 = (e) => {
        ...
        this.props.form.validateFields((err, values) => {
            if (!err) {
                ...
                ajax.post("/admin/autoloanApply/hasSend/list",data)
                    .then(response =>{
                        console.log(response);
                        if(response.code=="0"){
                            var datalist=response.result;
    ...
    changeStatus = (e, record) => {
        ...
        confirm({
            ...
            cancelText: "取消",
            onOk() {
                ajax.post("/admin/autoloanApply/send/confirm", {applyNo: e.applyNo})
                    .then(response => {
                        console.log(response);
                        if (response.code == "0") {
                            _this.getData();
    ...
    render() {
        ...
        const columns1 = [{
            ...
            render: (text, record) => <div  style={{color:'#1890FF',cursor:'pointer'}}
                                            onClick={this.changeStatus.bind(this, record)}>已寄件</div>
        }];
        const columns2 = [{
            title: '申请单号',
            dataIndex: 'applyNo',
        ...
        return(
            <div className="distributionList staffManage">
                <Breadcrumb style={{ padding: '16px 28px',background:"#fff"}}>
                    <Breadcrumb.Item>寄件登记</Breadcrumb.Item>
                </Breadcrumb>
