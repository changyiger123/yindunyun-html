...
class RiskQueryTdSimple extends React.Component {
    ...
    componentDidMount () {
        ...
        getPrice('anti_fraud_simple').then(res=>{
            if(res.code == "0") {
                this.setState({
                    priceData: res.data
                })
    ...
    go_search () {
        ...
        var url = '/admin/riskControlPlatform/simple/tongdun/rc/query'
        var data = {
    ...
    go_record() {
        window.location.hash = "/riskQueryRecord/2"
    }
                    ...
                    <Form className="editContent">
                        <div id="info" style={{padding:"80px 0"}}>
                            <div className="common_search_top">
                                <div>反欺诈报告简版查询</div>
                                <span>报告披露多头借贷详情、法院失信执行、借款人逾期、黑名单命中、社交关系等等，简版将不提供三要素核验、公安涉案、手机在网时长信息。</span>
                            </div>
                            <FormItem
                                label="姓名："
                                labelCol={{span: 6}}
...
const riskQueryTdSimple = Form.create()(RiskQueryTdSimple);
export default riskQueryTdSimple;