...
import V3Search from "../../components/v3Search/v3Search.jsx"
import ajax from "../../utils/ajax";

class RiskQueryEdit extends React.Component {
    ...
    go_search () {
        if (!this.state.realName) {
            ...
            return
        }else if (!this.state.bankCardCode && this.props.params.id == 2) {
            message.error("请输入银行卡号！");
            return
        }
        if (this.state.total < this.state.cost_money) {
            Modal.info({
                title: '余额不足',
        ...
        if (this.props.params.id == 1) {
            url = '/admin/riskControlPlatform/tongdun/rc/query'
        }else if (this.props.params.id == 2) {
            url = '/admin/riskControlPlatform/youfen/rc/query'
        }else if (this.props.params.id == 3) {
            url = '/admin/riskControlPlatform/miguan/rc/query'
        }
        var data = {}
        if (this.props.params.id == 1) {
            data = {
            ...
            onOk() {
                ...
                .then(response => {
                    if(response.code == "0") {
                        ...
                        if(_this.props.params.id == 1) {
                            $.showReport([json],_this.state.realName)
                        }else if (_this.props.params.id == 2) {
                            $.showReport1([json],_this.state.realName)
                        } else if (_this.props.params.id == 3) {
                            _this.setState({
                                previewVisible: true,
                                v3SearchData: json
                            })
        ...
        return (
            <div style={{height: '100%' }}>
                <Breadcrumb style={{padding: '16px 28px', background: "#fff", borderBottom: "1px solid #E8E8E8"}}>
                    <Breadcrumb.Item>{this.props.params.id=="1"?'多维度反欺诈报告查询':this.props.params.id=="2"?'银联信用报告查询':'大数据防控报告查询'}</Breadcrumb.Item>
                </Breadcrumb>
                <div style={{margin:'24px', minHeight: 'calc(100% - 111px)', background: '#fff'}}>
                    <Form className="editContent">
                        <div id="info" style={{padding:"80px 0"}}>
                            ...
                            {this.state.show_more_item && this.props.params.id == "1"?
                            ...
                            <div className="search_cost">
                                ...
                                查询<span> {this.props.params.id=="1"?'8':this.props.params.id=="3"?'6':'20'} </span>元/次，账户余额<span> {this.state.total}</span>元
                            </div>
                        </div>
                    </Form>
                </div>
...
const riskQueryEdit = Form.create()(RiskQueryEdit);
export default riskQueryEdit;