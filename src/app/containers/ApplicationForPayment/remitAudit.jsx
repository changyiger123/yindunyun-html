...
class applicationForm extends React.Component {
    ...
    componentWillMount= (e) =>{
        ...
        ajax.post("/admin/autoloanApply/waitRemitAudit/list",redata)
            .then(response =>{
                if(response.code=="0"){
        ...
        ajax.post("/admin/autoloanApply/afterFinanceAudit/list/all",data2)
            .then(response =>{
                if(response.code=="0"){
    ...
    handleSubmit1 = (e) => {
        ...
        this.props.form.validateFields((err, values) => {
            if (!err) {
                ...
                ajax.post("/admin/autoloanApply/waitRemitAudit/list",data1)
                    .then(response =>{
                        if(response.code=="0"){
    ...
    handleSubmit2 = (e) => {
        ...
        this.props.form.validateFields((err, values) => {
            if (!err) {
                ...
                ajax.post("/admin/autoloanApply/afterFinanceAudit/list/all",data)
                    .then(response =>{
    ...
    render() {
        ...
        const columns1 = [{
            ...
            key: 'auditReport',
            render:(text,record)=><Link to={'/ApplicationForPayment/riskReport/' + record.applyNo+'/'+1} name='auditReport'>查看</Link>
        }, {
            title: '操作',
            dataIndex: 'operation',
            key: 'operation',
            render:(text,record)=><Link to={'/ApplicationForPayment/financeAudit/'+record.applyNo}>{record.status==0?'审核':'补充资料审核'}</Link>
        }];
        const columns2 = [{
            title: '申请单号',
            dataIndex: 'applyNo',
            ...
            dataIndex: 'auditReport',
            key: 'auditReport',
            render:(text,record)=><Link to={'/ApplicationForPayment/remitPassOrNo/'+record.applyNo+'/'+record.status+'/'+1}>查看</Link>
        },];
        ...
        return(
            <div className="distributionList staffManage">
                <Breadcrumb style={{ padding: '16px 28px',background:"#fff"}}>
                    <Breadcrumb.Item>打款审核列表</Breadcrumb.Item>
                </Breadcrumb>
                {/*查询选项*/}
